...
void draw() {
  ...
  for(float i = 0.0; i < 30.0; i++){
      for(float j = 0.0; j < 30.0; j++){                  
          ...
          vertex((250.0/30.0) * i + x, (250.0/30.0)*j + y, z, i/30.0, j/30.0);
          vertex((250.0/30.0) * (i+1.0) + x, (250.0/30.0)*j + y, z, (i+1.0)/30.0, j/30.0);
          vertex((250.0/30.0) * (i+1.0) + x, (250.0/30.0)*(j+1.0) + y, z, (i+1.0)/30.0, (j+1.0)/30.0);
          vertex((250.0/30.0) * i + x, (250.0/30.0)*(j+1.0) + y, z, i/30.0, (j+1.0)/30.0);
          endShape();
          popMatrix();
      }
  }
